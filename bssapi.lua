...
function bsslib:getInventory()
    return require(game:GetService("ReplicatedStorage").Client.Systems.ClientStatCache):Get().Eggs
end

function bsslib.Pollen()
    return return game:GetService("Players").LocalPlayer.CoreStats.Pollen.Value
end

function bsslib:GetCooldown(mob)
    local cd = workspace.MonsterSpawners
    if cd:FindFirstChild(mob) then
        cd = cd:FindFirstChild("TimerAttachment"):FindFirstChild("TimerGui")
